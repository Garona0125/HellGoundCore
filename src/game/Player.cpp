...
{
	if(sGameEventMgr.IsActiveEvent(65))
	{	
...
{
	QueryResultAutoPtr result = AccountsDatabase.PQuery("SELECT `jifen` FROM `account` WHERE `account_id` = '%u'", GetSession()->GetAccountId());
	if (result)
	{
...
void Player::Modifyjifen(int32 d)
{
	...
	if (Newjifen < 0)
	{
		AccountsDatabase.PExecute("UPDATE `account` SET `jifen` = '0' WHERE `account_id` = '%u'", GetSession()->GetAccountId());
		AccountsDatabase.CommitTransaction();
	}
	else
	{
		AccountsDatabase.PExecute("UPDATE `account` SET `jifen` = '%u' WHERE `account_id` = '%u'", Newjifen, GetSession()->GetAccountId());
		AccountsDatabase.CommitTransaction();
	}
}
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, uint32 mount_id, Creature* npc)
{
	...
	if(m_session->GetPlayer()->HasItemCount(40014, 1))
	{
...
{
    QueryResultAutoPtr result = GameDataDatabase.PQuery("SELECT name FROM zizhi_name WHERE id = %u", id);
    if (result)
    {  
        Field *fields = result->Fetch();
